 ...
 */
public abstract class CesiumPropertyWriter<TDerived extends CesiumPropertyWriter<TDerived>> extends CesiumElementWriter implements
		ICesiumPropertyWriter {
	private String m_propertyName;
	private Lazy<CesiumIntervalListWriter<TDerived>> m_multipleIntervals;
	private Lazy<TDerived> m_interval;
	private ElementType elementType = ElementType.PROPERTY;
	...
	public CesiumPropertyWriter(String propertyName) {
		...
		m_multipleIntervals = new Lazy<cesiumlanguagewriter.CesiumIntervalListWriter<TDerived>>(
				new Func1<cesiumlanguagewriter.CesiumIntervalListWriter<TDerived>>(this, "createIntervalListWriter", new Class[] {}) {
					@Override
	...
	public final boolean getIsInterval() {
		return elementType == ElementType.INTERVAL || elementType == ElementType.PROPERTY_CONVERTED_TO_INTERVAL;
	}
	...
	public final void writeInterval(TimeInterval interval) {
		...
		getOutput().writeValue(
				CesiumFormattingHelper.toIso8601Interval(interval.getStart(), interval.getStop(),
						getOutput().getPrettyFormatting() ? Iso8601Format.EXTENDED : Iso8601Format.COMPACT));
	}

	@Override
	final protected void onOpen() {
		if (elementType == ElementType.INTERVAL || elementType == ElementType.PROPERTY_CONVERTED_TO_INTERVAL) {
	...
	final protected void onClose() {
		if (elementType == ElementType.INTERVAL || elementType == ElementType.PROPERTY_CONVERTED_TO_INTERVAL) {
			...
			if (elementType == ElementType.PROPERTY_CONVERTED_TO_INTERVAL) {
	...
	final protected void openIntervalIfNecessary() {
		if (elementType == ElementType.PROPERTY) {
			elementType = ElementType.PROPERTY_CONVERTED_TO_INTERVAL;
			onOpen();
		}
	}
	...
	final private TDerived copyForInterval() {
		...
		result.setElementType(ElementType.INTERVAL);
		return result;
	}
